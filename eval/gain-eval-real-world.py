...
datasets = ["elec", "weatherAUS", "covtype", "airlines", "pokerhand"]
...
for idx, dataset in enumerate(datasets):
    ...
    with open(gain_report_path, "w") as gain_report_out, open(gain_output, "w") as out:
        ...
        for metric in ["accuracy", "kappa"]:
            ...
            end = min([len(ecpf_ht_acc), len(ecpf_arf_acc), len(sarf_acc)])
...
for result in results:
    print(" & ".join([str(int(round(float(v)))) for v in result]))
